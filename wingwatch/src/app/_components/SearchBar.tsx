...
export default function SearchBar() {
  ...
  const handleClickOutside = (e: MouseEvent) => {
    if (
      dropdownRef.current &&
      !(dropdownRef.current as Element).contains(e.target as Node) &&
      inputRef.current &&
      !(inputRef.current as Element).contains(e.target as Node)
    ) {
      setIsDropdownVisible(false);
    }
  };
  ...
  const handleKeyDown = useCallback(
      ...
      switch (event.key) {
        case "ArrowDown":
          ...
          setHighlightedIndex((prevIndex) =>
            prevIndex < (placeSearchQuery.data?.length ?? 0) - 1
              ? prevIndex + 1
              : prevIndex,
          );
          break;
        ...
        case "Enter":
          ...
          if (highlightedIndex > -1) {
            const selectedId =
              placeSearchQuery.data![highlightedIndex]!.place_id;
    ...
    [isDropdownVisible, highlightedIndex, placeSearchQuery.data, handleSelect],
    ...
    <div ref={dropdownRef} style={{ position: "relative" }}>
      <input
        ...
        style={{
          ...
          width: "300px",
          borderRadius: "4px",
        }} // Extended the length of the search bar and rounded the borders
      />
      {isDropdownVisible &&
        placeSearchQuery.data &&
        placeSearchQuery.data.length > 0 && (
          <ul
            style={{
              listStyleType: "none",
              margin: 0,
